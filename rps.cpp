 ...
 * Winter 2023
 *
 * This program stimulates a game of rock-paper-scissors for two users to play. Furthermore, it has a game that is under construction, and it allows users to end the program after x amount of games.
 */
...
//main block of code where all to-call functions are implemented
...
void rps() {
    ...
    while(menuChoice == 1 || menuChoice == 2) {
        winnerOfGame = doGame(p1Name, p2Name, menuChoice);
            
        announceWinner(winnerOfGame);
...
//Prompts each user for their name
string getName(int playerNumber) {
    string p1Name;
    string p2Name;
    string name;
    ...
    if (playerNumber == 1) {
        ...
        //Error produced if improper name input
        if (p1Name.size() == 0) {
            printErrorMessage(1);
            
            //Player 1 defualt name if improper input name
            name = "Rocky";
        }
    }
    else {
        ...
        if (p2Name.size() == 0) {
            ...
            //Player 2 defualt name if improper input name
            name = "Creed";
        }
    }
...
//Displays menu options for player and obtain choice chosen by players
int getMenuChoice() {
    int choiceChosen;
    ...
    //If numeral input is beyond the range of options (3 < choice < 1), output
    //error message and reprompt players until numeral input is within range
    while (choiceChosen < 1 || choiceChosen > 3) {
...
//Decidec if input move is valid (r/R = rock, s/S = scissors, p/P = paper) and
//return whether the input was valid (true) or not valid (false)
bool isMoveGood(char move) {
    bool validMove;
    
    if (move == 'r' || move == 's' || move == 'p' || move == 'R' || move == 'S'
        || move == 'P') {
        validMove = true;
    }
    else {
...
//Prompts player to input a move and returns the player's move
char getMove(string playerName) {
    char move;
    ...
    //If input move is valid, then that move holds
    if (isMoveGood(move)) {
        move = move;
        cout << endl;
    }
    //If input move is not valid, then an error message will displayed,
    // and a default move (r for rock) will be assigned
    else {
        printErrorMessage(2);
        move = 'r';
    }
...
//Decides if Player 1 made a winning move
bool isRoundWinner(char move, char opponentMove) {
    bool p1Wins;
    
    //Rock beats scissors
    if ((move == 'r' || move =='R') && (opponentMove == 's' ||
                                        opponentMove =='S')) {
        p1Wins = true;
    }
    ...
    //If Player 2 made a winning move or if there was a draw, then function
    //returns false since Player 1 did not make a winning move
    else {
        p1Wins = false;
    }
...
//Outputs that a round was a draw or (if there was a winner)
//the winner of the round, depending on the input
void announceRoundWinner(string winnerName) {
    if (winnerName == "") {
...
//Stimulated one round of the game
int doRound(string p1Name, string p2Name) {
    char p1Move = getMove(p1Name);
    char p2Move = getMove(p2Name);
    bool p1Wins = isRoundWinner(p1Move, p2Move);
    int winnerOfRound;
    ...
    if (p1Wins == true) {
        ...
        if (tolower(p1Move) == tolower(p2Move)) {
            winnerOfRound = 0;
        }
        else {
            winnerOfRound = 2;
...
void announceWinner(string winnerName) {
    if (winnerName == "") {
        ...
        winnerName.append("!");
        cout << "Congratulations " << winnerName << endl << "You won EECS 183 Rock-Paper-Scissors!" << endl;
    }
...
string doGame(string p1Name, string p2Name, int gameType) {
    ...
    if (gameType == 1) {
        for (i = 0; i < MAX_ROUNDS; ++i) {
            winnerOfRound = doRound(p1Name, p2Name);
